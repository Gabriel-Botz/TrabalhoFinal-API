...
public class ListaFavoritosController {
    ...
    @ApiResponse(responseCode = "404", description = "Lista de favoritos não encontrada")
    @PutMapping("/{id}")
    public ResponseEntity<ListaFavoritosResponseDTO> atualizarListaFavoritos(@PathVariable UUID id,
            @RequestBody ListaFavoritosRequestDTO listaFavoritosRequestDTO) {
        ListaFavoritosResponseDTO listaAtualizada = listaFavoritosService.atualizarListaFavoritos(id,
                listaFavoritosRequestDTO);
